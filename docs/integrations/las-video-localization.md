...
「视频本地化」页的任务卡片集中展示：源视频名 → 成片素材名、计费状态（预授权冻结中／实际扣费 X 积分／对账中／预授权已释放）、失败原因与下一步操作。完成任务可直接预览（读取本地归档文件）与下载、下载 SRT 字幕、以成片继续翻译；失败任务可「按原参数重新提交」，回填阶段、档位与语言并自动重算报价，但始终创建新任务、新预授权，不复用旧幂等键。字幕文件登记在任务行的 `caption_local_path` 上，`mediaAuthorizationService` 已注册该路径的所有权映射：任务所有者可经 `/static` 下载，他人 404。
