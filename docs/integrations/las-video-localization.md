...
自动化测试使用隔离 SQLite、模拟 LAS 和模拟 TOS，覆盖认证、幂等、擦除→翻译、媒体归档、结算和重启读取。运行中的任务每 15 秒续期一次，两分钟内未续期的租约可由恢复轮询接管；提交结果不确定的任务转待对账。`backend-node/tools/seed-las-test-media.js` 仅通过项目 API 上传本地测试素材，运行前需设置 `LAS_SEED_USERNAME` 和 `LAS_SEED_PASSWORD`，不会提交 LAS 任务。
