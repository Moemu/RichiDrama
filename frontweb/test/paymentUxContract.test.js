...
test('account center exposes guarded two-channel recharge flow', async () => {
  ...
  assert.match(view, /选择充值套餐/)
  assert.match(view, /自定义充值/)
  assert.match(view, /const rechargeAmountChoice = ref\('199\.00'\)/)
  assert.match(view, /rechargeAmountChoice === 'custom'/)
  assert.match(view, /v-if="rechargeAmountChoice === 'custom'"/)
  assert.match(view, /paymentOptions\.value\.min_amount_yuan/)
  assert.match(view, /const rechargeAmountValid = computed/)
  assert.match(view, /:placeholder="rechargeAmountRange"/)
  assert.match(view, /const rechargeChannel = ref\(''\)/)
  assert.match(view, /提交充值订单/)
  assert.match(view, /payment-history\{[^}]*padding:1\.5rem[^}]*border:1px/)
  assert.match(view, /channel\.id === 'alipay' \? '支付宝' : '微信支付'/)
  assert.match(view, /QRCode\.toDataURL/)
  assert.match(view, /paymentSyncCounter % 5 === 0/)
  assert.match(view, /v-if="activePayment\.status === 'pending'"/)
  assert.match(view, /paymentQr\.value = ''[;\s]+stopPaymentTracking\(\)/)
  assert.match(view, /paymentCountdownTimer = window\.setInterval\(\(\) => \{ paymentTick\.value = Date\.now\(\) \}, 1000\)/)
