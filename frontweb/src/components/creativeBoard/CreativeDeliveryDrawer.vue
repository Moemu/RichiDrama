<template>
  <el-drawer :model-value="show" title="单集交付" size="min(640px, 100vw)" class="delivery-drawer" @update:model-value="$emit('update:show', $event)">
    <p>将已完成的片段按播放顺序排列。交付长度须为 1–3 分钟，片段短边至少 1080 像素。</p>
    <p v-if="deliveryIds.length" class="panel-hint">当前片段约 {{ Math.round(deliveryDurationMs / 1000) }} 秒。{{ deliveryDurationMs < 60000 ? '还需增加片段。' : deliveryDurationMs > 180000 ? '请移除部分片段。' : '可提交交付。' }}</p>
    ...
    <p v-if="!deliveryIds.length" class="empty-list">先选中一张已完成的视频卡片，点击“加入单集交付”。</p>
    <h3>字幕时间轴</h3><div v-for="(line, index) in subtitles" :key="index" class="subtitle-line"><el-input-number v-model="line.start" :min="0" :max="180" :precision="1" :step="0.1" /><span>至</span><el-input-number v-model="line.end" :min="0" :max="180" :precision="1" :step="0.1" /><el-input v-model="line.text" placeholder="字幕内容" /><el-button @click="subtitles.splice(index, 1)">×</el-button></div><el-button @click="subtitles.push({ start: 0, end: 2, text: '' })">＋ 字幕</el-button>
