<template>
  <el-dialog :model-value="modelValue" class="catalog-dialog discovery-dialog" title="获取模型" top="5vh" width="min(820px, 94vw)" append-to-body :close-on-click-modal="false" :before-close="close">
    <p class="discovery-intro">读取供应商列表，按模型能力导入。新型号保存为待上架，发布价格后才能使用。</p>
    ...
    <template #footer><span class="discovery-footer-note">保留已有模型、默认值和价格。</span><el-button :disabled="saving" @click="close()">取消</el-button><el-button type="primary" :disabled="!chosen.length || fetching || missingCapability" :loading="saving" @click="importModels">导入所选（{{ chosen.length }}）</el-button></template>
  </el-dialog>
</template>
