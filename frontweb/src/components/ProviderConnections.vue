  ...
  <section class="provider-connections" v-loading="loading">
    ...
    <article v-for="row in rows" :key="row.id" class="connection-card">
      ...
      <div class="connection-models" v-if="row.bindings.length">
        <div v-for="binding in row.bindings" :key="binding.id"><strong>{{ types[binding.service_type] || binding.service_type }}</strong><span>{{ binding.model.join('、') || '暂无模型' }}</span><el-button link @click="$emit('binding', binding.id)">模型调用设置</el-button></div>
      </div>
      <p v-else class="connection-key">导入模型后，它们会出现在模型目录中。</p>
      <footer><el-button :disabled="locked" @click="edit(row)">编辑连接</el-button><el-button :disabled="locked" @click="manual(row)">添加模型</el-button><el-button type="primary" :disabled="locked" @click="discover(row)">获取模型</el-button><el-button link @click="$emit('catalog')">模型目录与价格</el-button></footer>
    </article>
