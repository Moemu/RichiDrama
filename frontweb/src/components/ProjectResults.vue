<template>
  <section class="project-results" v-loading="loading">
    ...
    <p v-if="!filtered.length" class="empty">暂无成果。开始制作后，图片、视频和成片会在这里按分集整理。</p>
    <div class="results-grid"><article v-for="item in filtered" :key="`${item.type}-${item.id}`"><img v-if="item.type === 'image' && item.local_path" :src="url(item)" alt="生成图片" loading="lazy"/><video v-else-if="item.local_path" :src="url(item)" controls preload="metadata"/><div v-else class="result-placeholder">{{ statusLabel(item.status) }}</div><div class="result-copy"><b>{{ item.title || episodeTitle(item.episode_id) }}</b><span>{{ typeLabels[item.type] }} · {{ statusLabel(item.status) }}</span><small v-if="item.creator_name">{{ item.creator_name }}</small></div></article></div>
  </section>
</template>
<script setup>
...
const filtered = computed(() => results.value.filter(item => !episodeId.value || item.episode_id === episodeId.value).sort((a, b) => (a.episode_id || 0) - (b.episode_id || 0) || String(b.created_at || '').localeCompare(String(a.created_at || ''))))
...
const episodeTitle = id => props.episodes.find(episode => episode.id === id)?.title || '项目资源'
watch(() => [props.dramaId, projectSession.revision], async ([id]) => { loading.value = true; try { results.value = await request.get(`/dramas/${id}/collaboration/results`) } catch(error) { ElMessage.error(error.message) } finally { loading.value = false } }, { immediate: true })
</script>
<style scoped>
