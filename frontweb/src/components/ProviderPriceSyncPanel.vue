<template>
  <section class="price-sync-panel" aria-labelledby="price-sync-title">
    <header>
      <div><p>同步价目来源</p><h3 id="price-sync-title">价格同步与人工发布</h3><small>{{ scheduleNote }}</small></div>
      <div class="actions">
        <el-radio-group v-if="sources.length > 1" v-model="provider" size="small" @change="switchSource"><el-radio-button v-for="source in sources" :key="source.provider" :value="source.provider">{{ source.label }}</el-radio-button></el-radio-group>
        <el-button v-if="supportsProbe" :loading="probing" @click="probe">权限诊断</el-button><el-button type="primary" :loading="syncing" @click="runSync">立即同步</el-button>
      </div>
    </header>

    <el-alert v-if="supportsProbe && probeResult" :type="probeReady ? 'success' : 'warning'" :closable="false" show-icon>
    ...
    <div class="candidate-area">
        <div v-if="detail" class="detail-toolbar"><div><b>当前生效价目 → 最新{{ sourceLabel }}价</b><small>{{ formatTime(detail.fetched_at || detail.created_at) }} · {{ detail.changed_count }} 项变化 · {{ detail.mapped_count }}/{{ detail.candidate_count }} 已映射</small></div><el-button type="success" :disabled="!canCreateDraft" :loading="creatingDraft" @click="createDraft">生成价目草稿</el-button></div>
        ...
        <p v-else class="empty">尚无同步结果。点击“立即同步”读取当前{{ sourceLabel }}价格。</p>
    </div>
...
const currentSource = computed(() => sources.value.find((row) => row.provider === provider.value))
const sourceLabel = computed(() => currentSource.value?.label || '供应商')
const supportsProbe = computed(() => currentSource.value?.requires_source_check !== false)
const scheduleNote = computed(() => `${supportsProbe.value ? '每小时' : '每 6 小时'}只读检查。系统不会自动发布价格。`)
...
async function load() { const [latestSyncs, noticeRows, lastProbe] = await Promise.all([adminAPI.providerPriceSyncs({ limit: 1, provider: provider.value }), adminAPI.notices({ limit: 50 }), supportsProbe.value ? adminAPI.providerPriceProbeStatus(provider.value) : Promise.resolve(null)]); notices.value = noticeRows; probeResult.value = lastProbe; if (latestSyncs.length) await openSync(latestSyncs[0].id); else detail.value = null }
async function switchSource() { detail.value = null; probeResult.value = null; await load() }
...
async function probe() { probing.value = true; try { probeResult.value = await adminAPI.probeProviderPrices(provider.value); ElMessage.success(probeReady.value ? '只读权限诊断通过' : '诊断完成，请检查权限提示') } finally { probing.value = false } }
async function runSync() { syncing.value = true; try { const result = await adminAPI.syncProviderPrices(provider.value); await load(); await openSync(result.id); ElMessage.success(!actionableCandidates.value.length ? '价格没有变化' : result.status === 'unchanged' ? '供应商报价未变，请继续审核现有变化' : '价格同步完成，请人工审核') } finally { syncing.value = false } }
...
async function createDraft() { creatingDraft.value = true; try { draft.value = await adminAPI.createProviderPriceDraft(detail.value.id); publishForm.notify_users = true; publishForm.reason = `审核并发布${sourceLabel.value}价目同步批次 ${detail.value.id.slice(0, 8)}`; const changed = actionableCandidates.value.filter((row) => row.review_status === 'accepted').map(candidateChange); publishForm.notice_body = `生效时间：发布后立即生效。\n受影响价格：\n${changed.join('\n')}\n进行中的任务继续使用原价格快照。`; showPublish.value = true; emit('draft-created', draft.value) } finally { creatingDraft.value = false } }
...
// 来源清单只是文案与开关，拿不到时退回单一来源，不能连同步批次一起看不到
onMounted(async () => { sources.value = await adminAPI.providerPriceSources().catch(() => []); await load() })
</script>
