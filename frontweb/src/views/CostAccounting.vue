<template>
  <main class="cost-workspace" :aria-busy="busy">
    <header class="page-heading">
      <div><p class="eyebrow">瑞池传媒 · 运营核算</p><h1>消耗与成本</h1><p>按供应商提交时间核算；全部时间为北京时间。</p></div>
    ...
    <template v-if="tab === 'query'">
      ...
      <section v-if="detail" ref="detailPanel" class="panel detail" tabindex="-1">
        ...
        <dl><dt>业务操作</dt><dd>{{ detail.operation_id }}</dd><dt>调用尝试</dt><dd>{{ detail.id }} · 第 {{ detail.attempt }} 次</dd><dt>供应商 / 账号 / 配置 / 连接</dt><dd>{{ detail.provider || '未知' }} / {{ detail.account_id || '未确定' }} / {{ detail.config_id || '未知' }} / {{ detail.connection_id || '未使用共享连接' }}</dd><dt>供应商任务 / 请求</dt><dd>{{ detail.provider_task_id || '—' }} / {{ detail.provider_request_id || '—' }}</dd><dt>提交时间</dt><dd>{{ chinaTime(detail.submitted_at) }} · {{ detail.time_basis === 'authorization_approximation' ? '按原预授权时间近似' : '供应商提交时间' }}</dd></dl>
    ...
    <template v-if="tab === 'prices'">
      ...
      <section class="panel"><h2>价格版本审核</h2><p v-if="!prices.length">尚无成本价格版本。</p><article v-for="p in prices" :key="p.id" class="revision"><h3>#{{ p.id }} · {{ p.model }} · {{ p.status === 'published' ? '已发布' : '待审核' }}</h3><p>{{ accounts.items.find(a => a.id === p.account_id)?.name }} · {{ p.currency }} · {{ p.service_type }}</p><p>{{ chinaTime(p.effective_from) }} 至 {{ p.effective_to ? chinaTime(p.effective_to) : '长期有效' }} · 来源：{{ p.source }}</p><pre>{{ JSON.stringify(p.rules, null, 2) }}</pre><template v-if="p.status === 'draft'"><label class="checkbox"><input v-model="reviewed[p.id]" type="checkbox">已核对账号、原价与适用价格、单位、规格和生效时间</label><button :disabled="busy || !reviewed[p.id]" @click="publish(p.id)">确认发布此版本</button></template></article></section>
    </template>

    <template v-if="tab === 'backfill'">
      <section class="panel"><h2>历史补算</h2><p>先预览，再执行。只新增成本记录，保留原积分和任务状态。每批最多 1000 条历史汇总。</p><form class="inline-form" @submit.prevent="preview"><label>原账本结算开始日期<input v-model="backfillForm.date_from" type="date" required></label><label>原账本结算结束日期<input v-model="backfillForm.date_to" type="date" required></label><label>客户 ID（可选）<input v-model="backfillForm.organization_id" type="number" min="1"></label><button :disabled="busy">生成补算预览</button></form></section>
      <section v-if="batch" class="panel"><h2>批次预览</h2><p>{{ batch.preview.eligible }} 条可补算 · {{ batch.preview.skipped }} 条跳过。时间缺少提交证据时，明确使用原预授权时间近似。</p><div class="table-scroll" tabindex="0"><table><thead><tr><th>来源记录</th><th>模型 / 时间</th><th>结果</th><th>原因 / 成本估算</th></tr></thead><tbody><tr v-for="item in batch.preview.items" :key="item.source_key"><td>{{ item.source_id }}<small>{{ item.label }}</small></td><td>{{ item.call.model }}<small>{{ chinaTime(item.call.submitted_at) }}</small></td><td>{{ item.eligible ? '可补算' : '跳过' }}</td><td>{{ item.eligible ? costMoney(item.calculation.amount_micro) : item.reasons.join('；') }}</td></tr></tbody></table></div><p v-if="batch.result">已新增 {{ batch.result.inserted }} 条；执行时跳过 {{ batch.result.skipped.length }} 条。</p><template v-if="batch.status === 'preview'"><label class="checkbox"><input v-model="backfillConfirmed" type="checkbox">已检查预览及跳过原因，确认新增这些成本记录</label><button :disabled="busy || !backfillConfirmed" @click="execute">执行此批补算</button></template></section>
...
const backfillForm = reactive({ date_from: today.slice(0, 7) + '-01', date_to: today, organization_id: '' })
const priceForm = reactive({ account_id: '', model: '', service_type: 'text', currency: 'CNY', from: today + 'T00:00', to: '', source: '', rules: '[\n  { "meter": "output_token", "price": "0.01702", "unit_size": "1000" }\n]' })
