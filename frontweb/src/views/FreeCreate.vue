...
function formatHistoryTime(value) { return value ? new Date(value).toLocaleString() : '刚刚' }
...
function normalizeJob(data) { const generation = data.generation || {}; return { ...data, omni_job_id: data.id, status: generation.status || data.status || 'processing', error_msg: generation.error_msg || data.error_msg, videoUrl: localVideoUrl(generation) || data.video_url, duration: generation.duration || data.duration } }
...
async function extractFrame(position) { if (!activeJob.value?.omni_job_id || extractingPosition.value) return; extractingPosition.value = position; try { const asset = await omniVideoAPI.extractFrame(activeJob.value.omni_job_id, position); const item = { ...asset, alias: asset.name, usage: position === 'first' ? 'first_frame' : 'last_frame' }; assets.value.unshift(item); toggle(item); ElMessage.success(position === 'first' ? '首帧已提取到素材库，并设为当前镜头首帧' : '尾帧已提取到素材库，并设为当前镜头尾帧') } catch (error) { ElMessage.error(error.message || '提取视频帧失败') } finally { extractingPosition.value = '' } }
