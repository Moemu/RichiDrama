...
async function removeShot(shot) { try { await ElMessageBox.confirm(`删除“${shot.title || '未命名镜头'}”？`, '删除镜头', { type: 'warning' }); if (isProjectMode.value) { const next = shots.value.filter((item) => item.id !== shot.id); await storyboardsAPI.delete(shot.id); shots.value = next; await refreshProjectShots(next[Math.min(activeShotIndex.value, next.length - 1)]?.id); emit('changed'); return } await omniVideoAPI.deleteShot(sequence.value.id, shot.id); shots.value = shots.value.filter((item) => item.id !== shot.id); if (shot.id === activeShotId.value) loadShot(shots.value[Math.min(activeShotIndex.value, shots.value.length - 1)]) } catch (_) {} }
