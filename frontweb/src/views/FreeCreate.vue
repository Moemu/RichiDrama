  ...
  <section class="omni-page" :class="{ 'project-storyboard-page': isProjectMode, embedded: embedded }" @wheel.capture="containWorkbenchScroll">
    ...
    <section class="workbench" :class="`mobile-${mobileWorkspaceTab}`">
      ...
      <section class="center-stage" aria-label="当前镜头播放与时间线">
        ...
        <section v-else class="generation-stage-status" :class="{ 'is-processing': ['sd2_waiting','processing','upscale_pending','upscaling','interpolation_pending','interpolating','persisting'].includes(activeJob?.status), 'is-failed': activeJob && ['failed','retryable','invalid','billing_reconciliation','unknown'].includes(activeJob.status) }" :role="activeJob && ['failed','retryable','invalid','billing_reconciliation','unknown'].includes(activeJob.status) ? 'alert' : 'status'" aria-live="polite">
          <div class="generation-status-heading"><b>视频状态</b><el-tag :type="stageTagType" effect="dark">{{ stageLabel }}</el-tag></div>
          <template v-if="['sd2_waiting','processing','upscale_pending','upscaling','interpolation_pending','interpolating','persisting'].includes(activeJob?.status)"><b>{{ activeJob?.status === 'sd2_waiting' ? '真人素材认证准备中，完成后将自动生成' : generationProgressLabel }}</b><div class="generation-progress" role="status"><span><i :style="{ width: `${generationProgress}%` }"></i></span><em>{{ generationProgress }}%</em><small>{{ generationProgressMessage }}</small></div><el-button v-if="canCancelJob(activeJob)" size="small" type="warning" plain @click="cancelJob(activeJob)">取消未提交任务</el-button></template>
          ...
          <template v-else><b>尚未生成视频</b><small>完成生成后，播放器和成片操作将在这里显示。</small></template>
        </section>
