<template>
  <main class="page">
    ...
    <div v-show="accountTab === 'models'" class="account-view"><section class="panel models-panel"><h2>平台可用模型</h2><div v-if="models.length" class="model-list" role="list"><article v-for="m in models" :key="`${m.config_id}-${m.service_type}-${m.model}`" role="listitem"><span>{{ serviceLabel(m.service_type) }}</span><b>{{ m.model }}</b><small>{{ m.provider || '平台配置' }}</small></article></div><p v-else class="muted">管理员尚未配置可计费模型。</p></section></div>
...
.cards span, .muted, .panel-title p, .cards small { color: var(--text-muted); }
...
.page{width:100%;max-width:1200px;box-sizing:border-box;padding:clamp(1rem,3vw,2.5rem) 0 4rem;background:radial-gradient(48% 34% at 92% 0,color-mix(in srgb,var(--accent) 11%,transparent),transparent 72%)}.cards{gap:16px}.cards article,.panel{border-color:var(--border-subtle);border-radius:var(--radius-lg);background:color-mix(in srgb,var(--bg-surface) 92%,transparent);box-shadow:var(--shadow-sm)}.cards article:first-child{border-color:color-mix(in srgb,var(--accent) 42%,var(--border-color));background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 14%,var(--bg-surface)),var(--bg-surface))}@media(max-width:520px){.page{padding-inline:1rem}.header-actions :deep(.account-balance){display:none}.header-actions{width:100%;flex-wrap:wrap}.header-actions .el-button{flex:1}}
...
@media(max-width:45rem){.page{max-width:100%;padding:1rem}.account-tabs{width:100%;overflow-x:auto}.account-tabs button{flex:1 0 auto}.security-view{grid-template-columns:1fr}.overview-view{display:block}.overview-view .cards{grid-template-columns:1fr}}
...
@media(max-width:60rem){.overview-view{grid-template-columns:1fr}}
...
.models-panel{padding-inline:0}.model-list{display:grid;border-top:1px solid var(--border-subtle)}.model-list article{display:grid;grid-template-columns:minmax(8rem,.35fr) minmax(14rem,1fr) minmax(8rem,.35fr);align-items:center;gap:1rem;padding:1rem .25rem;border-bottom:1px solid var(--border-subtle)}.model-list span,.model-list small{color:var(--text-muted)}.model-list b{min-width:0;overflow-wrap:anywhere}
...
@media (prefers-reduced-motion:no-preference){.account-header,.account-tabs,.account-view{animation:account-enter var(--motion-normal,220ms) var(--motion-ease) both}.account-tabs{animation-delay:40ms}.account-view{animation-delay:80ms}}@keyframes account-enter{from{opacity:0;transform:translateY(.5rem)}to{opacity:1;transform:none}}
@media(max-width:45rem){.page{width:100%;max-width:100%;padding:1.25rem}.account-header{align-items:flex-start}.model-list article{grid-template-columns:1fr;gap:.3rem}}
