...
test('old personal logs import actual usage without account proof; reviewed scopes estimate through HTTP and survive restart', async () => {
  ...
  try {
    ...
    const estimate = await api('POST', '/reprices/preview', filters);
    assert.equal(estimate.preview.eligible, 116);
    assert.equal(estimate.preview.skipped, 106);
    assert.equal(estimate.preview.currencies[0].amount_micro, 637780248);
    assert.equal((await fixture.request('POST', `/admin/costs/reprices/${estimate.id}/execute`, { reason: '缺少确认' }, cookie)).status, 400);
    const first = estimate.preview.items.find(x => x.eligible);
    await api('POST', `/calls/${first.call_id}/reprice`, { reason: '预览后单条记录已更新' });
    const executed = await api('POST', `/reprices/${estimate.id}/execute`, { confirm: true, reason: '按已审核项目价格批量估算' });
    assert.equal(executed.result.updated, 115, 'changed revision is excluded');
    assert.equal(executed.result.skipped, 107);
    const revisionCount = db.prepare('SELECT COUNT(*) n FROM cost_revisions').get().n;
    await api('POST', `/reprices/${estimate.id}/execute`, { confirm: true, reason: '重复请求' });
    assert.equal(db.prepare('SELECT COUNT(*) n FROM cost_revisions').get().n, revisionCount);
    ...
    assert.equal((await api('GET', `/reprices/${estimate.id}`)).result.updated, 115);
    assert.equal((await api('POST', '/reprices/preview', filters)).preview.eligible, 0, 'calculated calls are excluded');
