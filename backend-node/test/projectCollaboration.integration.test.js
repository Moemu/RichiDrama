...
test('project HTTP permissions, concurrent text, copies and restart persistence', async t => {
  ...
  const { refreshProjectEditBaseline } = await import(require('node:url').pathToFileURL(path.resolve(__dirname, '../../frontweb/src/utils/projectEditBaseline.js')).href);
  ...
  const shotRevision = (await request(owner.token, 'GET', prefix)).body.data.revision;
  assert.equal((await request(owner.token, 'PUT', `/storyboards/${shotId}`, { description: '自己刚保存的分镜文字' })).status, 200);
  const editConfig = { method: 'put', data: { audio_volume: 0.7 }, headers: { 'X-Project-Revision': shotRevision } };
  const readProject = { get: async route => (await request(owner.token, 'GET', route)).body.data };
  await refreshProjectEditBaseline(readProject, editConfig, shotBaseline, ['', 'storyboards', String(shotId), ''], id);
  assert.ok(editConfig.headers['X-Project-Revision'] > shotRevision);
  assert.equal((await request(owner.token, 'PUT', `/storyboards/${shotId}`, editConfig.data, editConfig.headers)).status, 200);
  assert.equal((await request(editor.token, 'PUT', `/storyboards/${shotId}`, { audio_volume: 0.4 })).status, 200);
  const conflictingEdit = { method: 'put', data: { audio_volume: 0.9 }, headers: { 'X-Project-Revision': shotRevision } };
  await refreshProjectEditBaseline(readProject, conflictingEdit, shotBaseline, ['', 'storyboards', String(shotId), ''], id);
  assert.equal(conflictingEdit.headers['X-Project-Revision'], shotRevision);
  assert.equal((await request(owner.token, 'PUT', `/storyboards/${shotId}`, conflictingEdit.data, conflictingEdit.headers)).status, 409);
