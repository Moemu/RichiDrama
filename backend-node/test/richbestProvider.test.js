...
test('relay Seedance 2.0 Mini always omits camera_fixed instead of sending true or false', () => {
  ...
  assert.equal(i2v.body.camera_fixed, undefined, 'mini 图生视频也会被中转拒绝，只能省略');
  assert.ok(i2v.dropped.includes('camera_fixed'));
  ...
  assert.equal(full.body.camera_fixed, true, '仅 mini 模型省略');
});
