...
const scenarios = [
  { activations: require('./helpers/volcengineMiniSeedream4Prices.json'), inputPrices: [20, 40, 80], outputPrices: [200, 400, 800], imagePrices: [20, 20] },
  { activations: require('./helpers/volcengineProSeedream45Prices.json'), inputPrices: [320, 384, 960], outputPrices: [1600, 1920, 4800], imagePrices: [19.5, 25] },
];
const ai = require('../src/services/aiConfigService');
const prices = require('../src/services/providerPriceService');

for (const { activations, inputPrices, outputPrices, imagePrices } of scenarios) {
  const textModel = activations[0].FoundationModelName;
  const imageModel = activations[1].FoundationModelName;
  test(`${textModel} and ${imageModel} sync, publish and quote through HTTP after restart`, async () => {
    ...
    try {
      ...
      for (const [service_type, model] of [['text', textModel], ['image', imageModel], ['storyboard_image', imageModel]]) {
      ...
      assert.deepEqual(input.new_conditions.usage_tiers.map(tier => tier.unit_price_points), inputPrices);
      assert.deepEqual(output.new_conditions.usage_tiers.map(tier => tier.unit_price_points), outputPrices);
      ...
      for (const row of rows.filter(row => row.meter === 'image')) assert.deepEqual(row.new_conditions.rates.map(rate => rate.unit_price_points), imagePrices);
      ...
      for (const [tokens, tier] of [[32768, 0], [32769, 1], [131072, 1], [131073, 2], [262144, 2]]) {
        const quote = await f.request('POST', '/billing/quotes', { service_type: 'text', model: textModel, usage: { input_token: tokens, output_token: 1000 } }, cookie);
        assert.equal(quote.status, 200, JSON.stringify(quote.body));
        assert.equal(quote.body.data.amount, Math.round((tokens * inputPrices[tier] / 1000000 + outputPrices[tier] / 1000) * 10000) / 10000);
      }
      assert.equal((await f.request('POST', '/billing/quotes', { service_type: 'text', model: textModel, usage: { input_token: 262145 } }, cookie)).status, 400);
      for (const service_type of ['image', 'storyboard_image']) for (const has_image_input of [false, true]) {
        const quote = await f.request('POST', '/billing/quotes', { service_type, model: imageModel, usage: { image: 1 }, pricing_context: { has_image_input } }, cookie);
        assert.equal(quote.status, 200, JSON.stringify(quote.body));
        assert.equal(quote.body.data.amount, imagePrices[has_image_input ? 0 : 1]);
      }
  ...
  test(`${textModel} and ${imageModel} reject incomplete or changed pricing shapes`, async () => {
    const f = await modelCatalogFixture();
    try {
