...
function list(db, query = {}) { const deleted = query.deleted === '1' || query.deleted === true; const type = query.tool_type; const owner = query.owner_user_id; const args=[]; let sql=`SELECT * FROM tool_runs WHERE deleted_at ${deleted ? 'IS NOT NULL' : 'IS NULL'}`; if(type){sql+=' AND tool_type=?';args.push(type)} if(owner){const access=require('./projectAccessService');sql+=' AND ((owner_user_id=?'+(access.installed(db)?' AND NOT EXISTS (SELECT 1 FROM project_collaboration c JOIN dramas d ON d.id=c.drama_id AND d.deleted_at IS NULL WHERE c.drama_id=tool_runs.drama_id)':'')+')'+(access.installed(db)?' OR drama_id IN ('+access.projectIdsSql(db,owner)+')':'')+')';args.push(Number(owner))} return db.prepare(sql+' ORDER BY updated_at DESC,id DESC').all(...args).map(row); }
