...
function create(db, body) {
  ...
  let out;
  try {
    out = db.prepare('INSERT INTO tool_runs (tool_type,batch_id,title,model,language,status,input_json,owner_user_id,billing_authorization_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(body.tool_type, body.batch_id || null, body.title || '', body.model || null, body.language || 'zh', body.status || 'pending', JSON.stringify(input), body.owner_user_id || null, body.billing_authorization_id || null, now, now);
  } catch (error) {
