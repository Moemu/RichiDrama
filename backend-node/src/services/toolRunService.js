...
function create(db, body) {
  ...
  const out = db.prepare('INSERT INTO tool_runs (tool_type,batch_id,title,model,language,status,input_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)').run(body.tool_type, body.batch_id || null, body.title || '', body.model || null, body.language || 'zh', body.status || 'pending', JSON.stringify(input), now, now);
...
function list(db, query = {}) { const deleted = query.deleted === '1' || query.deleted === true; const type = query.tool_type; return db.prepare(`SELECT * FROM tool_runs WHERE deleted_at ${deleted ? 'IS NOT NULL' : 'IS NULL'} ${type ? 'AND tool_type=?' : ''} ORDER BY updated_at DESC,id DESC`).all(...(type ? [type] : [])).map(row); }
function set(db, id, values) { const old = get(db, id, true); if (!old) throw new Error('工具运行不存在'); const now = stamp(); db.prepare('UPDATE tool_runs SET status=?, output_json=?, streamed_text=?, error_msg=?, continuation_count=?, updated_at=?, completed_at=? WHERE id=?').run(values.status ?? old.status, values.output !== undefined ? JSON.stringify(values.output) : JSON.stringify(old.output), values.streamed_text ?? old.streamed_text, values.error_msg ?? null, values.continuation_count ?? old.continuation_count, now, values.status === 'completed' ? now : old.completed_at, old.id); return get(db, id, true); }
...
async function executeAnalysis(db, log, id, onDelta) { const run = get(db,id,true); const input = run.input; set(db,id,{ status:'processing', streamed_text:'' }); let text=''; try { text=await aiClient.streamGenerateText(db,log,'text',analysisPrompt(input),'你是专业漫剧策划与剧本分析师。', {model:run.model || undefined, temperature:.4}, (delta)=>{ text+=delta; db.prepare('UPDATE tool_runs SET streamed_text=?,updated_at=? WHERE id=?').run(text,stamp(),id); if(onDelta) onDelta(delta); }); let output; try { output=JSON.parse(text.replace(/^```json\s*|```$/g,'').trim()); } catch (_) { output={ raw_json:text }; } return set(db,id,{status:'completed',output,streamed_text:text}); } catch(error) { set(db,id,{status:'failed',error_msg:error.message,streamed_text:text}); throw error; } }
async function executeStory(db, log, id) { const run=get(db,id,true); set(db,id,{status:'processing'}); try { const output=await storyGeneration.generateStory(db,log,{...run.input,model:run.model}); return set(db,id,{status:'completed',output}); } catch(error) { set(db,id,{status:'failed',error_msg:error.message}); throw error; } }
async function executeReverse(db, log, id) { const run=get(db,id,true); const asset=run.assets[0]; if(!asset) throw new Error('请选择要反推的素材'); set(db,id,{status:'processing'}); try { const path=require('path'); const cfg=require('../config').loadConfig(); const root=path.resolve(process.cwd(),cfg.storage?.local_path||'./data/storage'); const abs=asset.local_path?path.resolve(root,asset.local_path):null; const question=`按${run.language==='en'?'English':run.language==='bilingual'?'中英文双语':'中文'}输出：主体、构图、镜头、色彩光影、风格、负面约束和完整提示词。`; let result;
    if(asset.type==='image') result=await aiClient.generateTextWithVision(db,log,'vision',question,'你是视觉提示词分析师。',{localAbsPath:abs||undefined,imageUrl:asset.url},{model:run.model||undefined,max_tokens:1200});
...
function importDrama(db, log, id, body={}) { const run=get(db,id); if(!run) throw new Error('工具运行不存在'); const output=run.output || {}; const title=body.title || run.title || output.overview?.title || 'AI 导入项目'; const drama=dramaService.createDrama(db,log,{title,description:output.overview?.summary || ''}); if(Array.isArray(output.episodes)) dramaService.saveEpisodes(db,log,drama.id,{episodes:output.episodes.map((ep,i)=>({episode_number:ep.episode_number || ep.episode || i+1,title:ep.title || `第${i+1}集`,script_content:ep.content || ep.script || ''}))}); return drama; }
