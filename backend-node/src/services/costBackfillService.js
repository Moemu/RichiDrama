...
function candidates(db, input) {
  ...
  return rows.map(row => {
    ...
    const usage = ledger.normalizeUsage(parse(row.usage_json), row.service_type);
    ...
    const call = {
      ...
      organization_id: row.organization_id, customer_kind: row.organization_id ? 'customer' : evidence.personal_account === true ? 'personal' : 'unknown',
      ...
      account_id: evidence.account_id, config_id: evidence.config_id || null, connection_id: evidence.connection_id || null, provider: evidence.provider || null,
    ...
    if (!call.account_id) reasons.push('缺少历史供应商账号证据');
    if (call.customer_kind === 'unknown' || !call.user_name || call.organization_id && !call.organization_name) reasons.push('缺少历史归属快照');
    ...
    if (calculation.cost_status !== 'calculated') reasons.push(calculation.cost_status === 'missing_usage' ? '缺少规则要求的计量证据' : '缺少有效期内的审核价格或规格');
    return { source_id: row.id, source_key: source, label: '历史汇总记录', eligible: !reasons.length, reasons,
