...
function usages(db, assetId) {
  ...
  if (!asset?.drama_id) return [];
  const result = [];
  const samePath = value => !!asset.local_path && normalizeMediaReference(value) === normalizeMediaReference(asset.local_path);
  ...
  const rows = db.prepare('SELECT s.*,e.title episode_title FROM storyboards s JOIN episodes e ON e.id=s.episode_id WHERE e.drama_id=? AND e.deleted_at IS NULL AND s.deleted_at IS NULL').all(asset.drama_id);
  for (const row of rows) {
    let ids = []; try { ids = JSON.parse(row.omni_asset_ids || '[]') } catch (_) {}
    if ([...ids.map(value => Number(value?.asset_id ?? value?.id ?? value)), Number(row.omni_first_frame_asset_id), Number(row.omni_last_frame_asset_id)].includes(asset.id) || referenceFields.some(field => samePath(row[field]))) {
      result.push({ episode_id: row.episode_id, episode_title: row.episode_title, storyboard_id: row.id });
    }
  }
  for (const kind of ['characters', 'scenes', 'props']) {
    for (const row of db.prepare(`SELECT * FROM ${kind} WHERE drama_id=? AND deleted_at IS NULL`).all(asset.drama_id)) {
      if (referenceFields.some(field => samePath(row[field]))) result.push({ resource_type: kind, resource_id: row.id });
    }
  }
  for (const link of db.prepare("SELECT resource_type,resource_id FROM asset_resource_links WHERE drama_id=? AND asset_id=? AND status='active'").all(asset.drama_id, asset.id)) result.push(link);
  return result;
}
...
function decorate(db, asset) {
  if (!asset?.drama_id || !access.installed(db)) return asset;
  const source = db.prepare('SELECT c.source_asset_id,c.added_by,u.display_name added_by_name FROM project_asset_copies c LEFT JOIN users u ON u.id=c.added_by WHERE c.asset_id=?').get(asset.id);
  return { ...asset, project_source: source || null, usages: usages(db, asset.id) };
}

module.exports = { join, prepareReferences, usages, assertRemovable, decorate };
