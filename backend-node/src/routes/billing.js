...
module.exports = function billingRoutes(db) {
  return {
    ...
    usage: (req, res) => response.success(res, billing.pagedUsage(db, { ...req.query, user_id: req.auth.role === 'admin' && req.query.user_id ? req.query.user_id : req.auth.id })),
    transactions: (req, res) => response.success(res, billing.pagedTransactions(db, { ...req.query, user_id: req.auth.role === 'admin' && req.query.user_id ? req.query.user_id : req.auth.id })),
