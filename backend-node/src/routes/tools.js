...
function routes(db, log) {
  ...
  return {
    ...
    execute:(req,res)=>{try{const body=req.body||{};const fn={script_analysis:toolRuns.executeAnalysis,script_analysis_stream:toolRuns.executeAnalysis,script_writing:toolRuns.executeStory,reverse_prompt:toolRuns.executeReverse}[req.params.type]; if(!fn) return response.badRequest(res,'此工具请使用既有图片或视频生成接口'); const run=toolRuns.create(db,{tool_type:req.params.type,title:body.title,model:body.model,language:body.language,input:body.input||body,assets:body.assets||[]}); setImmediate(()=>fn(db,log,run.id).catch(err=>log.error('tool run failed',{id:run.id,error:err.message}))); response.created(res,run);}catch(e){sendError(res,e)}},
