  ...
  return {
    list: (req, res) => {
      try {
        ...
        const { items, total, page, pageSize } = sceneLibraryService.listLibraryItems(db, query, req.auth);
    ...
    create: (req, res) => {
      try {
        const item = sceneLibraryService.createLibraryItem(db, log, req.body || {}, req.auth);
    ...
    get: (req, res) => {
      try {
        const item = sceneLibraryService.getLibraryItem(db, req.params.id, req.auth);
    ...
    update: (req, res) => {
      try {
        const item = sceneLibraryService.updateLibraryItem(db, log, req.params.id, req.body || {}, req.auth);
        if (!item) return response.notFound(res, '场景库项不存在');
        response.success(res, item);
      } catch (err) {
    ...
    delete: (req, res) => {
      try {
        const ok = sceneLibraryService.deleteLibraryItem(db, log, req.params.id, req.auth);
