...
module.exports = function routes(db, log, cfg) { return {
  ...
  create(req, res) { try {
    ...
    if (sequence && !sequence.drama_id && Number.isInteger(dramaId) && dramaId > 0) db.prepare('UPDATE omni_video_sequences SET drama_id=?, updated_at=? WHERE id=?').run(dramaId, new Date().toISOString(), sequence.id);
  ...
  async cancel(req, res) { try { response.success(res, await omniVideoService.cancelJob(db, log, req.params.id, req.auth)); } catch (err) { response.badRequest(res, err.message); } },
  ...
  extractFrame(req, res) { try { response.created(res, require('../services/omniFrameService').extract(db, cfg, log, req.params.id, req.body?.position, req.auth)); } catch (err) { response.badRequest(res, err.message); } },
  extractVideoFrame(req, res) { try { response.created(res, require('../services/omniFrameService').extractVideoGeneration(db, cfg, log, req.params.id, req.body?.position, req.auth)); } catch (err) { response.badRequest(res, err.message); } },
