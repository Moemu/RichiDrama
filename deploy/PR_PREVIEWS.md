...
- 媒体呈现走两层：热拷贝（`/data/minidrama-data/storage`）以 OverlayFS 只读下层即时可见；**已归档 OSS 的冷对象按需回源**——受信容器 `minidrama-preview-media-proxy` 持有生产凭据（仅经部署时 env-file 注入，永不进入预览应用），挂内网接受应用的 `remote_read_base_url` 请求并流式转发（Range/206 全透传）。写操作全部落在预览私有的上层目录；生产字节与 OSS 对象零改动。两个受信组件（edge=鉴权边界、media-proxy=凭据出口）均不发布端口，预览应用保持单宿无外网。
