...
- 媒体真实度由 OverlayFS 联合视图提供：冷填充层（服务器侧凭据只经宿主进程，从 OSS 限量拉取快照引用而本地已裁剪的对象）在上、`/data/minidrama-data/storage` 热拷贝目录在下，共同作为只读下层挂进预览存储路径；写操作全部落在预览私有的上层目录（生产字节与 OSS 对象零改动）。默认预算：2GiB / 400 个对象 / `library/` 前缀优先（可用 `MINIDRAMA_PREVIEW_MEDIA_MAX_BYTES|LIMIT_COUNT|PREFIXES` 调整），预算外或拉取失败的对象按 404 呈现。候选镜像按发布裁剪策略在每次预览成功后回收。
