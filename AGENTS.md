...
- 验证优先使用现有测试、静态检查和本地 UI。允许为本次验收执行必要的本地登录、只读请求，以及使用隔离测试数据的项目 API 测试。执行前检查配置和测试隔离措施，核实这些路径不会调用真实供应商、产生费用或修改生产数据；不能仅凭本地地址或 dev 档作此判断。真实生成和生产操作继续遵守本文件的计费与授权规则；禁止为方便而直接调用供应商 API。
- 远端检查门槛适用于本次任务包含的提交、PR 或部署。仅审查和仅本地修改不因此获得提交、推送或部署授权。提交交付须确认目标远端分支已包含该提交，且该提交所需的 CI 检查通过。
- 涉及部署的任务，只有在 **Git 提交已推送到目标远端分支，且该提交对应的部署检查/工作流显示成功** 后才可宣称上线成功。记录并跟踪本次交付的确切提交。部署检查失败、缺失或仍在运行时，即使服务器容器健康、页面可访问，部署仍未完成。检查成功后，再确认部署环境运行目标提交、服务健康检查通过；必要时验证受影响页面或接口。
- 遇到无法在已授权范围内解决的权限、平台或检查配置阻塞时，报告证据和所需操作，并明确交付仍未完成。继续不依赖该阻塞的已授权工作。没有新证据或状态变化时，不反复触发相同检查。
